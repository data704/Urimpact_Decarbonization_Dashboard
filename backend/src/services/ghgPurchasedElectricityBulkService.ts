...
import { validate, purchasedElectricityFormSchema } from '../utils/validators.js';

type ExcelHeader = (typeof PURCHASED_ELECTRICITY_EXCEL_HEADERS)[number];

export type PurchasedElectricityRowInput = {
  activityType: string;
  sourceType: string;
  consumption: number;
...
function findHeaderRow(aoa: unknown[][]): { headerRow: number; colIdx: Record<ExcelHeader, number> } {
  const needed = PURCHASED_ELECTRICITY_EXCEL_HEADERS.map((h) => normalizeExcelHeader(h));
  const maxScan = Math.min(aoa.length, 250);
  for (let r = 0; r < maxScan; r++) {
    const row = (aoa[r] ?? []) as unknown[];
    const colIdx: Partial<Record<ExcelHeader, number>> = {};
    let ok = true;
    for (let hi = 0; hi < PURCHASED_ELECTRICITY_EXCEL_HEADERS.length; hi++) {
      ...
      colIdx[PURCHASED_ELECTRICITY_EXCEL_HEADERS[hi]] = j;
    }
  ...
  throw new Error(
    `Could not find the data header row (columns: ${PURCHASED_ELECTRICITY_EXCEL_HEADERS.join(', ')}). Use the "Purchased Electricity" sheet from the official template.`
  );
}
...
function readWorkbookMatrix(buffer: Buffer, originalName: string): unknown[][] {
  ...
  if (lower.endsWith('.csv')) {
    ...
    sheet = wb.Sheets[PURCHASED_ELECTRICITY_SHEET_NAME];
    if (!sheet) {
      throw new Error(
        `Workbook has no sheet named "${PURCHASED_ELECTRICITY_SHEET_NAME}". Use the official template file.`
      );
    }
  }
...
/** Excel serial date → YYYY-MM-DD string for display & downstream parsing. */
...
function isProbablyDataRow(input: PurchasedElectricityRowInput): boolean {
...
export type ParsedPurchasedElectricityBulkRow = { excelRow: number; input: PurchasedElectricityRowInput };

/** One row result for preview (no DB / no Climatiq). */
export type PurchasedElectricityBulkPreviewRow = {
  excelRow: number;
  status: 'valid' | 'invalid';
  errors: string[];
  input: PurchasedElectricityRowInput;
  mappedPreview?: {
    siteName: string;
    activityType: string;
...
export function evaluatePurchasedElectricityBulkRow(
  excelRow: number,
  input: PurchasedElectricityRowInput
): PurchasedElectricityBulkPreviewRow {
  ...
  try {
    const mapped = mapPurchasedElectricityToGhgBody(
      {
...
export function buildPurchasedElectricityBulkPreview(parsed: ParsedPurchasedElectricityBulkRow[]): PurchasedElectricityBulkPreviewRow[] {
  return parsed.map(({ excelRow, input }) => evaluatePurchasedElectricityBulkRow(excelRow, input));
}

/**
 * Parse the "Purchased Electricity" sheet (or CSV) into template rows with 1-based Excel row numbers.
 */
export function parsePurchasedElectricityBulkFile(buffer: Buffer, originalName: string): ParsedPurchasedElectricityBulkRow[] {
  const aoa = readWorkbookMatrix(buffer, originalName);
  const { headerRow, colIdx } = findHeaderRow(aoa);
  const out: ParsedPurchasedElectricityBulkRow[] = [];

  for (let r = headerRow + 1; r < aoa.length; r++) {
    const row = (aoa[r] ?? []) as unknown[];
    if (!row.some((c) => String(c ?? '').trim())) continue;
    const excelRow = r + 1;
    let input: PurchasedElectricityRowInput;
    try {
