...
import { validate, purchasedHeatingFormSchema } from '../utils/validators.js';

type ExcelHeader = (typeof PURCHASED_HEATING_EXCEL_HEADERS)[number];

export type PurchasedHeatingRowInput = {
  activityType: string;
  sourceType: string;
  consumption: number;
...
function findHeaderRow(aoa: unknown[][]): { headerRow: number; colIdx: Record<ExcelHeader, number> } {
  ...
  throw new Error(
    `Could not find the data header row (columns: ${PURCHASED_HEATING_EXCEL_HEADERS.join(', ')}). Use the "Purchased Heating" sheet from the official template.`
  );
}
...
function readWorkbookMatrix(buffer: Buffer, originalName: string): unknown[][] {
  ...
  if (lower.endsWith('.csv')) {
    ...
    sheet = wb.Sheets[PURCHASED_HEATING_SHEET_NAME];
    if (!sheet) {
      throw new Error(
        `Workbook has no sheet named "${PURCHASED_HEATING_SHEET_NAME}". Use the official template file.`
      );
    }
  }
...
function isProbablyDataRow(input: PurchasedHeatingRowInput): boolean {
...
export type ParsedPurchasedHeatingBulkRow = { excelRow: number; input: PurchasedHeatingRowInput };

export type PurchasedHeatingBulkPreviewRow = {
  excelRow: number;
  status: 'valid' | 'invalid';
  errors: string[];
  input: PurchasedHeatingRowInput;
  mappedPreview?: {
    siteName: string;
    activityType: string;
...
export function evaluatePurchasedHeatingBulkRow(
  excelRow: number,
  input: PurchasedHeatingRowInput
): PurchasedHeatingBulkPreviewRow {
  const v = validate(purchasedHeatingFormSchema, { ...input, dataEntryChannel: 'BULK_UPLOAD' });
  if (!v.success) {
    return { excelRow, input, status: 'invalid', errors: v.errors ?? ['Invalid row'] };
  }
  try {
    const mapped = mapPurchasedHeatingToGhgBody(
      {
...
export function buildPurchasedHeatingBulkPreview(parsed: ParsedPurchasedHeatingBulkRow[]): PurchasedHeatingBulkPreviewRow[] {
  return parsed.map(({ excelRow, input }) => evaluatePurchasedHeatingBulkRow(excelRow, input));
}

export function parsePurchasedHeatingBulkFile(buffer: Buffer, originalName: string): ParsedPurchasedHeatingBulkRow[] {
  const aoa = readWorkbookMatrix(buffer, originalName);
  const { headerRow, colIdx } = findHeaderRow(aoa);
  const out: ParsedPurchasedHeatingBulkRow[] = [];

  for (let r = headerRow + 1; r < aoa.length; r++) {
    const row = (aoa[r] ?? []) as unknown[];
    if (!row.some((c) => String(c ?? '').trim())) continue;
    const excelRow = r + 1;
    let input: PurchasedHeatingRowInput;
