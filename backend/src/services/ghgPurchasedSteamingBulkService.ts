...
import { validate, purchasedSteamingFormSchema } from '../utils/validators.js';

type ExcelHeader = (typeof PURCHASED_STEAMING_EXCEL_HEADERS)[number];

export type PurchasedSteamingRowInput = {
  activityType: string;
  sourceType: string;
  consumption: number;
...
function findHeaderRow(aoa: unknown[][]): { headerRow: number; colIdx: Record<ExcelHeader, number> } {
  ...
  throw new Error(
    `Could not find the data header row (columns: ${PURCHASED_STEAMING_EXCEL_HEADERS.join(', ')}). Use the "Purchased Steaming" sheet from the official template.`
  );
}
...
function readWorkbookMatrix(buffer: Buffer, originalName: string): unknown[][] {
  ...
  if (lower.endsWith('.csv')) {
    ...
    sheet = wb.Sheets[PURCHASED_STEAMING_SHEET_NAME];
    if (!sheet) {
      throw new Error(
        `Workbook has no sheet named "${PURCHASED_STEAMING_SHEET_NAME}". Use the official template file.`
      );
    }
  }
...
function isProbablyDataRow(input: PurchasedSteamingRowInput): boolean {
...
export type ParsedPurchasedSteamingBulkRow = { excelRow: number; input: PurchasedSteamingRowInput };

export type PurchasedSteamingBulkPreviewRow = {
  excelRow: number;
  status: 'valid' | 'invalid';
  errors: string[];
  input: PurchasedSteamingRowInput;
  mappedPreview?: {
    siteName: string;
    activityType: string;
...
export function evaluatePurchasedSteamingBulkRow(
  excelRow: number,
  input: PurchasedSteamingRowInput
): PurchasedSteamingBulkPreviewRow {
  const v = validate(purchasedSteamingFormSchema, { ...input, dataEntryChannel: 'BULK_UPLOAD' });
  if (!v.success) {
    return { excelRow, input, status: 'invalid', errors: v.errors ?? ['Invalid row'] };
  }
  try {
    const mapped = mapPurchasedSteamingToGhgBody(
      {
...
export function buildPurchasedSteamingBulkPreview(parsed: ParsedPurchasedSteamingBulkRow[]): PurchasedSteamingBulkPreviewRow[] {
  return parsed.map(({ excelRow, input }) => evaluatePurchasedSteamingBulkRow(excelRow, input));
}

export function parsePurchasedSteamingBulkFile(buffer: Buffer, originalName: string): ParsedPurchasedSteamingBulkRow[] {
  const aoa = readWorkbookMatrix(buffer, originalName);
  const { headerRow, colIdx } = findHeaderRow(aoa);
  const out: ParsedPurchasedSteamingBulkRow[] = [];

  for (let r = headerRow + 1; r < aoa.length; r++) {
    const row = (aoa[r] ?? []) as unknown[];
    if (!row.some((c) => String(c ?? '').trim())) continue;
    const excelRow = r + 1;
    let input: PurchasedSteamingRowInput;
