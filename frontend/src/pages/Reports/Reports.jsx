...
function computeReportOutputs({ config, totalTonnes, scope1Tonnes, scope2Tonnes, breakdownRaw, baseYear }) {
    ...
    const assumptions = [
        { assumption_id: 'scope', label: 'Scope boundary', value: 'Scope 1 & 2 only', unit: null, source: 'GHG Protocol', notes: 'Scope 3 excluded by design' },
    ...
    useEffect(() => {
        ...
        const sections = [
            ...
            },
            {
                ...
                bindings: { base_year: o.period.base_year, target_year: o.period.target_year, years_to_target: o.pathway.years_to_target, tier_label: o.tier.label },
            },
            {
                    ...
                    <>
                        ...
                        <div className="report-print-page">
                            <section className="card report-section">
                                ...
                                <div className="methodology-list" style={{marginTop:'1rem'}}>
                                    ...
                                    <p className="body-text"><strong>Scope:</strong> Scope 1 &amp; 2 only. Scope 3, financial projections, and engineering feasibility are excluded from this model by design.</p>
                                </div>
                            </section>
                            <div className="report-page-footer">{getPageNum('S8_strategy')} of {totalPages} | Generated by URIMPACT Platform</div>
                        </div>

                        {/* ── S9: Implementation Roadmap ──────────────────────────────────── */}
                        <div className="report-print-page">
                            <section className="card report-section">
                                <div className="section-title">S9 — Implementation Roadmap</div>
                                {narrText('roadmap.caption') && (
                                    <p className="body-text lead">{narrText('roadmap.caption')}</p>
                                )}
                                <div className="roadmap-phases">
                                    <p className="body-text"><strong>{outputs?.period?.base_year ? outputs.period.base_year + 1 : '—'} — Foundation Year</strong></p>
                                    ...
                                    <p className="body-text"><strong>{outputs?.period?.base_year ? outputs.period.base_year + 2 : '—'} — Scale-Up</strong></p>
                                    <ul className="body-text bullet-list">
                                    ...
                                    <p className="body-text"><strong>{outputs?.period?.base_year ? outputs.period.base_year + 3 : '—'}–{outputs?.period?.target_year ? outputs.period.target_year - 1 : '—'} — Acceleration</strong></p>
                                    <ul className="body-text bullet-list">
                                        <li>Progressive fleet electrification</li>
                                        <li>Energy efficiency Phase 2</li>
                                        <li>Establish carbon removal procurement pipeline</li>
                                    </ul>
                                    <p className="body-text"><strong>{outputs?.period?.target_year ?? '—'} — Target Year</strong></p>
                                    <ul className="body-text bullet-list">
                                        <li>Achieve {fmt(outputs?.pathway?.target_emissions)} tCO₂e residual ({outputs?.tier?.label})</li>
                                        <li>Secure verified carbon removal contracts ({fmt(outputs?.residual?.removal_requirement_tco2e_per_year)} tCO₂e/yr)</li>
                                        <li>Third-party verification &amp; public disclosure</li>
                                    </ul>
                                </div>
                                <div className="roadmap-timeline">
                                    {outputs && [...new Set([outputs.period.base_year+1, outputs.period.base_year+2, outputs.period.base_year+3, outputs.period.target_year-1, outputs.period.target_year])].map((y, i) => <span key={`rt-${i}-${y}`}>{y}</span>)}
                                </div>
                            </section>
                            ...
                            <section className="card report-section">
                                ...
                                {narrText('assumptions.bullets') ? (
                                    ...
                                    <ol className="body-text assumptions-list">
                                        ...
                                        <li>Scope boundary is Scope 1 &amp; 2 only. Scope 3, financial projections, and engineering feasibility are excluded by design.</li>
                        ...
                        <div className="report-print-page">
                            <section className="card report-section">
                                ...
                                <div className="section-title sub" style={{marginTop:'1.25rem'}}>Run Manifest</div>
                                <table className="report-table calculation-table appendix-factors">
                                    <tbody>
                                        <tr><td>Engine Version</td><td>{outputs?.manifest?.engine_version}</td></tr>
                                        <tr><td>Policy Version</td><td>{outputs?.manifest?.policy_version}</td></tr>
                                        <tr><td>Generated At</td><td>{outputs?.manifest?.generated_at ? new Date(outputs.manifest.generated_at).toLocaleString() : '—'}</td></tr>
                                        <tr><td>Input Hash</td><td><code>{outputs?.manifest?.input_hash}</code></td></tr>
                                        <tr><td>QA: Baseline &gt; 0</td><td>{outputs?.qa_checks?.baseline_positive ? '✓ Pass' : '✗ Fail'}</td></tr>
                                        <tr><td>QA: Base &lt; Target Year</td><td>{outputs?.qa_checks?.base_before_target ? '✓ Pass' : '✗ Fail'}</td></tr>
                                        <tr><td>QA: Residual &lt; (1−R)</td><td>{outputs?.qa_checks?.residual_below_target ? '✓ Pass' : '✗ Fail'}</td></tr>
                                    </tbody>
                                </table>
                                ...
                                <ul className="body-text bullet-list">
                                    ...
                                    <li><strong>Accuracy:</strong> Primary meter readings and invoices</li>
                                    <li><strong>Consistency:</strong> Standardized data collection and validation</li>
                                </ul>
                            </section>
